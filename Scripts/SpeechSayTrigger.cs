...
public class SpeechSayTrigger : MonoBehaviour {
	public enum TriggerMode { TOUCH, TRIGGERZONE }
	public TriggerMode triggerMode = TriggerMode.TRIGGERZONE;

	public string[] triggerWords;
	//public QuickPhoto[] quickPhoto;
	public VolumeFiller volumeFiller;
	public TriggerZone triggerZone;
	//public PhotonSyncBox photonSyncBox;
	...
	void Start() {
		...
		if (textMesh == null) textMesh = GameObject.FindGameObjectWithTag("TextDisplay").GetComponent<TextMesh>();
		textMeshRen = textMesh.GetComponent<Renderer>();
	...
	void Update() {
		...
		if (Time.realtimeSinceStartup > markTriggerTime + 3f) {
			markTriggerTime = 0f;
		}

		if (triggerMode == TriggerMode.TOUCH ){
			hasBeenTriggered = Cardboard.SDK.Triggered;
		} else if (triggerMode == TriggerMode.TRIGGERZONE) {
			if (markTriggerTime == 0f) {
				hasBeenTriggered = triggerZone.trigger;
				markTriggerTime = Time.realtimeSinceStartup;
			}
		}

		if (!_isListening && hasBeenTriggered) {// && cardboardInputController.directionVector == Vector3.zero) {
			_isListening = true;
			textMesh.text = "...";
			textMeshRen.enabled = true;
		...
		if (textMeshRen.enabled && Time.realtimeSinceStartup > markTime + textTimeout) {
			textMesh.text = "";
			textMeshRen.enabled = false;
		}
		...
		for (int i=0; i<triggerWords.Length; i++) {
			if (result.ToLower() == triggerWords[i]) {
				//triggerZone.trigger = true;
				//photonSyncBox.syncBoxes[0] = true;
				textMesh.text = triggerWords[i];
				// * * *
				rememberTriggerWord = triggerWords[i];
				// * * *
				//volumeFiller.quickPhoto = quickPhoto[i];
				//volumeFiller.photosName = quickPhoto[i].name;
				//volumeFiller.armInit = true;
				triggerFinal = true;
			}
		}
		if (armMainTrigger) {
			volumeFiller.go = triggerFinal;
			armMainTrigger = false;
		}
	}
